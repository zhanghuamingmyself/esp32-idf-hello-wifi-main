...
static uint8_t heart_rate_val[2] = {0, 1};
static uint8_t led_status[2] = {0, 2};
...
static void heart_gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_ADD_CHAR_EVT:
        ESP_LOGI(TAG, "Characteristic add, status %d, attr_handle %d, char_uuid %x",
        ...
        ESP_LOGI(TAG, "heart rate char handle %d", param->add_char.attr_handle);
    ...
    case ESP_GATTS_ADD_CHAR_DESCR_EVT:
        ESP_LOGI(TAG, "Descriptor add, status %d, attr_handle %u",
    ...
    case ESP_GATTS_WRITE_EVT:
        ESP_LOGI(TAG, "Characteristic write, value len %u, value ", param->write.len);
        ...
        if (gl_profile_tab[HEART_PROFILE_APP_ID].descr_handle == param->write.handle && param->write.len == 2) {
            ...
            if (descr_value == 0x0001) {
                ...
                ESP_LOGE(TAG, "Invalid descriptor value");
                ESP_LOG_BUFFER_HEX(TAG, param->write.value, param->write.len);
            }
        }
    ...
    case ESP_GATTS_START_EVT:
        ESP_LOGI(TAG, "Service start, status %d, service_handle %d", param->start.status, param->start.service_handle);
        break;
    ...
    case ESP_GATTS_SET_ATTR_VAL_EVT:
        ESP_LOGI(TAG, "Attribute value set, status %d", param->set_attr_val.status);
        if (indicate_enabled) {
...
static void auto_io_gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_ADD_CHAR_EVT:
        ...
        break;
    case ESP_GATTS_ADD_CHAR_DESCR_EVT:
        ESP_LOGI(TAG, "Descriptor add, status %d", param->add_char_descr.status);
...
void ble_server_init(void)
{
    ...
    if (ret) {
        ESP_LOGE(TAG, "app register error, error code = %x", ret);
        return;
    }
